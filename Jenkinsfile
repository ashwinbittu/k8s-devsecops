...
pipeline {
  ...
  environment {
    ...
    applicationURL="http://3.25.149.86"
      ...
      stage('Docker Build & Push') {
            steps {
              withDockerRegistry([credentialsId: "dockerhub-ashwinbittu", url: ""]){
                sh 'printenv'
                sh 'sudo docker build -t ashwinbittu/numeric-app:""$VERSION"" .'
                sh 'docker push ashwinbittu/numeric-app:""$VERSION""'
              }
            }
        }
            ...
            steps {
              parallel(
                ...
                "Rollout Status": {
                  withKubeConfig([credentialsId: 'kubeconfig']) {
                    echo "Disabling for the timebeing"
                    sh 'kubectl get pods -n k8s-devsecops'
